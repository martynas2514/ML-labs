...
```
Number of genes selected when threshold = `r bestTh` : `r numberOfGenes`
...
res <- predict(cv.elastic, newx = as.matrix(test[ ,-ncol(test)]), s = cv.elastic$lambda.min, type = "class")
```

```{r , echo=FALSE}
knitr::kable(table(testdata$y,res))
```

Test error: `r sum(testdata$y != res)`

```{r , echo=FALSE}
library("kernlab")

x <- as.matrix(unname(train[ ,-ncol(train)]))
svp = ksvm(x,train$CellType, type="C-svc", kernel='vanilladot')

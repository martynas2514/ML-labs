...
```
Centroid plot shows distance from gene mean value to global mean value. Positive or negative shows if mean is greater or smaller than the global mean.  Yes, it can happen that all values for some gene in centroid plot are positivive. Number of genes selected when threshold = `r bestTh` : `r numberOfGenes`.
...
confusionMatrix <- pamr.predict(model, testdata$x, threshold = bestTh)
...
```
Test error: `r sum(testdata$y != confusionMatrix)`
...
Table bellow shows test error and amount of features used.
```{r , echo=FALSE}
results <- matrix(c(sum(testdata$y != confusionMatrix), numberOfGenes), nrow = 1)
results <- rbind(results,c(sum(testdata$y != res1), cv.elastic$glmnet.fit$df[which(cv.elastic$glmnet.fit$lambda == cv.elastic$lambda.min)]))
results <- rbind(results, c(sum(testdata$y != res2), nrow(mydata$x)))
row.names(results) <- c("NSC", "EN", "SVM")
colnames(results) <- c("Test error", "N. features")
knitr::kable(results)
```
